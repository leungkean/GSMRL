...
def decoder(x, mean, sigma):
    ...
    """
    return tf.transpose((1/(2*sigma**2)) * tf.math.reduce_sum((x - mean)**2, axis=2))
...
class VGMM_z(tf.keras.Model):
    def __init__(self, d, k=2, nhidden_layers=3, hidden_size=256):
        ...
        """
        self.logits = tf.Variable( tf.random.normal(mean=0., stdev=0.1, shape=(k,)) , name='logits')  # TODO
